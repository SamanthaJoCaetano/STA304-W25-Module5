...
`svymean()` function in the `survey` library to run an estimation of the mean mass of characters. Assume it was a Simple Random Sample and use the finite population correction with $N=224$.
...
Let's try taking this a step further playing around with some Stratified random sampling ratio estimation. Use the `svymean()` function in the `survey` library to run a ratio estimation of mass given height. Assume the study instead collected the data by stratifying by species, where in the population there 16 droids, 80 humans and 128 other species. Use the finite population correction with $N=200$ to estimate the mean mass.
...
For $\mu$: $\bar{y}_{st} \pm z_{\alpha/2} \sqrt{\sum_{h=1}^HW_h^2(1-\frac{n_h}{N_h}) s^2_h/n_h}$
