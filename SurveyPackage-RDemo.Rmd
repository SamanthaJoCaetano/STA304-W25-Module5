...
let's categorize species to be `human`, or `other`.
...
Let's start by playing around with some SRS ratio estimation. Use the
`svymean()` function in the `survey` library to run an estimation of the
mean mass of characters. Assume it was a Simple Random Sample and use
the finite population correction with $N=224$.
...
Additionally, we can use `confint()` to calculate the confidence
interval:
...
Use the `svyratio()` function in the `survey` library to run a ratio
estimation of mass given height. Assume it was a Simple Random Sample
and use the finite population correction with $N=224$.
...
Let's try taking this a step further playing around with some Stratified
random sampling ratio estimation. Use the `svymean()` function in the
`survey` library to run a ratio estimation of mass given height. Assume
the study instead collected the data by stratifying by species, where in
the population there 16 droids, 80 humans and 128 other species. Use the
finite population correction with $N=200$ to estimate the mean mass.

```{r}
table(starwars_clean$species_clean)

# In the sample there are 4 droids, 20 humans and 32 other species.

# Need to include fpc variable (with pop size for each strata)
# Need to include a weight variable (with pop/samp size for each strata)
...
svymean(~mass, design=strata.design)
confint(svymean(~mass, design=strata.design))
```

Let's check this calculation by hand. Recall for stratified sampling CI:

For $\mu$:
$\bar{y}_{st} \pm z_{\alpha/2} \sqrt{\sum_{h=1}^HW_h^2(1-\frac{n_h}{N_h}) s^2_h/n_h}$

Note: $\bar{y}_{st} = \sum_{h=1}^{H}W_h\bar{y}_h$
...
Nh = c(16, 80, 128)
W = Nh/N
ybar = c(69.75, 81.31, 112.1875)
s2 = c(2604.2500, 373.7325, 52661.8708) 

ybar_str = sum(W*ybar)
SE_ybar = sqrt( sum(W^2*(1-nh/Nh)*s2/nh) )
...
Use the `lm()` function in the `survey` library to run a linear
regression estimation of mass given height.
